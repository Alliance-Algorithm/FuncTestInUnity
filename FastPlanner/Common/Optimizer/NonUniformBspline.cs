    ...
    public class NonUniformBspline
    {
        ...
        public void BuildTimeLine(float TimeInterval = 0.1f)
        {
            ...
            for (int i = 0; i < n_; i++)
            {
                ...
                if (acc_t > limit_acc)
                    timeDensity[i] *= (float)Math.Sqrt(acc_t / limit_acc);
            }
        ...
        public (int Id, bool Safe) Check(float t, ESDFMap CostMap, Vector2 Position)
        {
            ...
            for (t += 0.1f; t < t_[k + 1]; t += 0.1f)
            {
                ...
                temp = CalaPosition(u, k);
                var t2 = CostMap.Vector22XY(temp);
